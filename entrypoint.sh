...
OPENBOX=$!
x11vnc -display "$DISPLAY" -forever -shared -nopw -rfbport 5900 >"$HOME/x11vnc.log" 2>&1 &
X11VNC=$!
websockify --web=/usr/share/novnc 6080 localhost:5900 >"$HOME/novnc.log" 2>&1 &
NOVNC=$!
trap 'kill "$NOVNC" "$X11VNC" "$OPENBOX" "$XVFB" 2>/dev/null || true' EXIT
sleep 2
