...
if [[ -z "${CMCC_ARCHIVE_URL:-}" && -z "${CMCC_ARCHIVE_PATH:-}" && -z "${GITHUB_TOKEN:-}" ]]; then
  if [[ -t 0 ]]; then
...
step 1 "准备项目压缩包"
if [[ -n "${CMCC_ARCHIVE_PATH:-}" && -f "$CMCC_ARCHIVE_PATH" ]]; then
  echo "使用本地压缩包：$CMCC_ARCHIVE_PATH"
  cp -f "$CMCC_ARCHIVE_PATH" "$ARCHIVE"
elif [[ -n "${CMCC_ARCHIVE_URL:-}" ]]; then
  echo "使用直链下载：$CMCC_ARCHIVE_URL"
  curl --fail --location --show-error --progress-bar --retry 3 \
    -o "$ARCHIVE" "$CMCC_ARCHIVE_URL"
else
