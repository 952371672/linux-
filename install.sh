...
step() { printf '\n==== [%s] %s ====\n' "$1" "$2"; }
progress_download() { local url="$1" out="$2"; local part="${out}.part"; rm -f "$out" "$part"; curl --http1.1 -fL --retry 5 --retry-all-errors --retry-delay 2 --connect-timeout 30 --speed-time 60 --speed-limit 1024 --max-time 1800 --progress-bar --show-error "$url" -o "$part"; [[ -s "$part" ]] || { echo '下载结果为空' >&2; return 1; }; mv -f "$part" "$out"; printf '\n下载完成：%s\n' "$out"; }
step 1 '检查运行权限与系统依赖'
